...
if isempty(NData)
    ...
    if hyd==1
        ...
        NData.hvcL = colorbar(colorbarposition);
    ...
    if isfield(NData,'hvc'), ylabel(NData.hvc, nodeYlabel,'fontsize',nodeFontYlabel); end
...
if ~isempty(titleinfo)
    if d.getMSXTimeStep == 300
        if mod(frame, 12) == 0
            title(gca, {[figtitle,' ', num2str(frame/12), ' hours'], ''},'fontsize',figtitleFontSize);
        end
    else
        title(gca, {[figtitle,' ', num2str(frame*60), ' seconds'], ''},'fontsize',figtitleFontSize);
    end
end
